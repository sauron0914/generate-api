...
class StandardDataType extends Contextable {
    ...
    getDefName(originName) {
        ...
        if (this.isDefsType) {
            name = (originName && this.typeName !== 'ObjectMap') ? `${originName}.${this.typeName}` : `${this.typeName}`;
        }
        return name;
    }
    getEnumType() {
...
class Interface extends Contextable {
    ...
    getParamsCode(className = 'Params', surrounding = utils_1.Surrounding.typeScript) {
        return !!this.parameters.length ? `interface ${className} {
      ${this.parameters
  ...
  ` : '';
    ...
    getRequestParams(surrounding = utils_1.Surrounding.typeScript) {
        const paramList = this.getParamList(capitalizeFirstLetter(this.name)+'Params').filter(param => param.paramType);
